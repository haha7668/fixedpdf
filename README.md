...
- **双语对照翻译**（三种模式，设置面板可切换）：
  - **浮层模式**：中文译文以半透明浮层出现在**每一段原文的正下方**；
  - **逐段对照模式**：页面底部按「原文段落 → 译文段落」逐条对照；
  - **流式重排模式**：隐藏原图，原文与译文交错排版，适合通读。
- **按需 + 预测翻译**：只翻译可见页，自动预翻译下方页面（`IntersectionObserver`）。
- **翻译缓存持久化**：整页缓存 `translation_cache.json`、段落级缓存 `segment_cache.json`，按书存储到对应文件夹，二次打开秒加载、不重复调用 AI。
...
### 4. 智能模型路由
- 有图片 → 自动用「视觉模型」；无图片 → 用默认模型（如 `deepseek-chat`）。
- 翻译任务与默认模型保持一致，避免用推理/视觉模型做翻译（又慢又贵）。
